...
export default function ProductCard({ item, score, hogeMatchDrempel, bezoekerLocatie }: ProductCardProps) {
  ...
  return (
    <Link href={`/items/${item.id}`} className="product-kaart">
      <div className="product-kaart__afbeelding">
        ...
        <span className={`match-badge${score >= hogeMatchDrempel ? ' match-badge--hoog' : ''}`}>
          {score}% match
        </span>
      </div>
